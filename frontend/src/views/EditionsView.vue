...
async function searchActiveBibs() {
  const edition = raceStore.activeEdition
  if (!edition) return
...
function toggleActiveSearch() {
  activeExpanded.value = !activeExpanded.value
  if (!activeExpanded.value) {
    activeResults.value = []
    searchName.value = ''
  }
}
...
<template>
  <section class="relative z-10 pt-32 pb-20 px-6 max-w-6xl mx-auto">
    ...
    <template v-else>
      <!-- Edición actual -->
      <div v-if="raceStore.activeEdition" class="mb-16">
        ...
        <div class="grid md:grid-cols-2 gap-8 items-stretch bg-gris-oscuro border border-white/5 p-6 md:p-8">
          ...
          <div class="flex flex-col">
            <div class="font-barlow-condensed font-black text-6xl md:text-7xl text-naranja mb-2">
              {{ raceStore.activeEdition.year }}
            </div>
            <div class="font-barlow-condensed font-bold text-2xl uppercase leading-tight mb-2">
              {{ raceStore.activeEdition.name }}
            </div>
            <div class="text-gris-texto mb-6">
              {{ raceStore.activeEdition.date }} &middot; {{ raceStore.activeEdition.location }}
            </div>

            <div class="flex flex-wrap gap-3 mb-6">
              <a
                v-if="raceStore.activeEdition.resultsUrl"
                :href="raceStore.activeEdition.resultsUrl"
                target="_blank"
              ...
              <RouterLink
                :to="`/galeria?edicion=${raceStore.activeEdition.id}`"
              ...
              <button
                v-if="raceStore.activeEdition.showBibSearch"
                type="button"
                class="font-barlow-condensed font-bold text-xs tracking-widest uppercase text-white/70 border border-white/20 px-4 py-2 hover:border-naranja hover:text-naranja transition-colors text-center"
            ...
            <div v-if="raceStore.activeEdition.showBibSearch && activeExpanded" class="mt-auto">
      ...
      <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div
            v-for="edition in raceStore.editions.filter(e => e.id !== raceStore.activeEdition?.id)"
            :key="edition.id"
